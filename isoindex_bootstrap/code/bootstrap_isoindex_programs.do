...
program define simulatevisits
	...
	syntax using/, outputfile(string) instances(integer)

tempfile tf_collect tf_user_trips tf_predvisits
use userid venue using "../input/trips_est.dta", clear
duplicates drop
gen int trip = 1
collapse (sum) trips_user = trip, by(userid)
merge 1:1 userid using "../input/users_est.dta", assert(using match) keep(match) keepusing(userid_num race_nd) nogen
keep if race_nd==0
...
program define isoindices_simulateddraws
	...
	syntax using/, [tableoutputfile(string)] [dtaoutputfile(string)] draws(integer)

	//Load simulated trips, merge with user-level racial data, throw out second+ reviews
	tempfile tf_draws_clean tf_cumulate
	use "`using'", clear
	merge m:1 userid_num using "../input/users_est.dta", assert(using match) keep(match) keepusing(asian black whithisp) nogen
