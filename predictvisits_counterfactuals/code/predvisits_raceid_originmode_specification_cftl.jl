...
specification_temp = ARGS[3]
cftl_temp = ARGS[4]

for i in procs()
  @spawnat i global const race = race_temp
...
@everywhere path_predvisitsarray = string("../input/predictedvisitsarray/",cftl,"/")
...
elseif cftl == "socialchange"
  @everywhere data_estimates = CSV.read(string(path_estimates,"socialchange_parameters_",race,".csv"),allowmissing=:none,weakrefstrings=false)
end
...
@time save(string(path_output,"predictedvisits_collapsed_over_workplace_",race,"_",originmode,"_",specification,"_",cftl,".jld"),"predvisit",predvisits_collapsed_over_workplace,compress=true)
