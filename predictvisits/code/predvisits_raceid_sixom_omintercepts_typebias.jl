...
for i in procs()
  ...
  @spawnat i global const bias = bias_tmp
end
...
@everywhere spec_neither,spec_nosocial,spec_nospatial,spec_standard = prepare_spec_variations(norg,originmode,specification,data_estimates;bias=bias)
...
@time list_predvisits = pmap(file->begin
  ...
  if specification == "omintercepts" && bias == "nobias"
    #create the origin mode intercepts
    df = create_originmodeintercept(df)
  end

  df_spec_neither = compute_prob_visit_venue(norg,df,data_estimates,spec_neither;fastestmode=fastestmode,mintime=mintime)
  df_spec_nosocial = compute_prob_visit_venue_originmodeintercept(norg,df,data_estimates,spec_nosocial,bias;fastestmode=fastestmode,mintime=mintime)
  df_spec_nospatial = compute_prob_visit_venue(norg,df,data_estimates,spec_nospatial;fastestmode=fastestmode,mintime=mintime)
  df_spec_standard = compute_prob_visit_venue_originmodeintercept(norg,df,data_estimates,spec_standard,bias;fastestmode=fastestmode,mintime=mintime)
...
@time save(string(path_output,"predictedvisits_collapsed_over_workplace_",race,"_",originmode,"_",specification,"_",bias,".jld"),"predvisit",predvisits_collapsed_over_workplace,compress=true)
