...
##Table 4
echo '\begin{table}[!h] \caption{Restaurant fixed effects} \label{tab:Restaurant-fixed-effects} \vspace{-3mm}' > ../output/tabsfigs/table4.tex
echo '\begin{center} \input{../input/tab_venueFE_sixom.tex}' >> ../output/tabsfigs/table4.tex
echo '\begin{minipage}[t]{0.9\textwidth}' >> ../output/tabsfigs/table4.tex
echo "{\footnotesize \textsc{Notes}: Each column reports an estimated conditional-logit model of individuals' decisions to visit a Yelp venue. Standard errors in parentheses. Statistical significance denoted by \textit{a} (1\%), \textit{b} (5\%), \textit{c} (10\%). The unreported covariates are restaurant fixed effects. \par}" >> ../output/tabsfigs/table4.tex
echo '\end{minipage}' >> ../output/tabsfigs/table4.tex
echo '\end{center} \end{table}' >> ../output/tabsfigs/table4.tex

cp ../input/tab_venueFE_sixom.tex ../output/tabsfigs/tab_venueFE_sixom.tex

##Table 5
echo '\begin{table}[!h] \caption{Observables versus restaurant fixed effects} \label{tab:LR-test-venue-FEs} \vspace{-7mm} \begin{center} '  > ../output/tabsfigs/table5.tex
cat  ../input/table_observables_vs_FE.tex >> ../output/tabsfigs/table5.tex
echo -e '\n \\begin{minipage}{.95\\textwidth}' >> ../output/tabsfigs/table5.tex
echo '{\footnotesize \textsc{Notes}:' >> ../output/tabsfigs/table5.tex
echo 'The observables specifications include 60 covariates, while the fixed-effect specifications include 14 covariates plus the 10,945 restaurant fixed effects.' >> ../output/tabsfigs/table5.tex
echo 'Thus, the $\chi^{2}$ test has 10,899 degrees of freedom.' >> ../output/tabsfigs/table5.tex
echo 'Statistical significance denoted by $a$ (1\%), $b$ (5\%), $c$ (10\%).\par}' >> ../output/tabsfigs/table5.tex
echo '\end{minipage} \end{center} \end{table}'  >> ../output/tabsfigs/table5.tex

##Table 6
cat ../input/dissimilarity_venues_mainspec.tex > ../output/tabsfigs/table6.tex
...
##Figure 8
##cat ../input/figure_dumbo.tex > ../output/tabsfigs/figure8.tex
...
#cat  ../input/tab_robustnesschecks2_asian_sixom.tex >> ../output/tabsfigs/table_A04_b.tex
...
#echo '\begin{table} \caption{Six-origin-mode specifications: Black reviewers (part 2)} \label{tab:black_robust2_sixom}' > ../output/tabsfigs/table_A05_b.tex
...
echo '\begin{center} \vspace{-4mm} \resizebox{.95\textwidth}{!}{' >> ../output/tabsfigs/table_A05_b.tex
#cat  ../input/tab_robustnesschecks2_black_sixom.tex >> ../output/tabsfigs/table_A05_b.tex
...
#echo '\begin{table} \caption{Six-origin-mode specifications: White/Hispanic reviewers (part 2)} \label{tab:whithisp_robust2_sixom}' > ../output/tabsfigs/table_A06_b.tex
...
echo '\begin{center} \vspace{-4mm} \resizebox{.95\textwidth}{!}{' >> ../output/tabsfigs/table_A06_b.tex
#cat  ../input/tab_robustnesschecks2_whithisp_sixom.tex >> ../output/tabsfigs/table_A06_b.tex
...
##cat  ../input/table_homeonlysample_homeonly.tex >> ../output/tabsfigs/table_A11.tex
...
cat ../input/dissimilarity_tracts_mainspec.tex > ../output/tabsfigs/table_A14.tex


echo '\begin{table} \caption{Consumption segregation robustness checks} \label{tab:dissimilarity:robustnesschecks}'  > ../output/tabsfigs/table_A15.tex
echo '\vspace{-8mm} \begin{center} \resizebox{1\textwidth}{!}{' >> ../output/tabsfigs/table_A15.tex
cat  ../input/dissimilarity_robustnesschecks_venue_level_comp_across_race.tex >> ../output/tabsfigs/table_A15.tex
echo '}' >> ../output/tabsfigs/table_A15.tex
echo '\begin{minipage}[t]{1.0\textwidth}' >> ../output/tabsfigs/table_A15.tex
echo '{\footnotesize \textsc{Notes}:' >> ../output/tabsfigs/table_A15.tex
echo 'This table reports dissimilarity indices computed using estimated preference parameters, as in the upper panel of Table \ref{tab:dissimilarity:bootstrap}.'>> ../output/tabsfigs/table_A15.tex
echo 'Column 1 uses the coefficients from columns four to six of Table \ref{tab:mainestimates}, as in Table \ref{tab:dissimilarity:bootstrap}.' >> ../output/tabsfigs/table_A15.tex
echo 'Columns 2 and 3 use columns one to three and four to six of Table \ref{tab:nestedlogit}, respectively.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 4  uses the coefficients from the specification with origin-mode-specific intercepts reported in Table \ref{tab:estimates:omintercepts}.' >> ../output/tabsfigs/table_A15.tex
echo 'Columns 5 and 6 use coefficients from specifications in which observations are limited to the first half and first fifth of NYC restaurant reviews posted by each reviewer, respectively.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 7 and 8 use coefficients from specifications in which the estimation sample is split into reviewers with residences located using information contained in one or two reviews in column 7 and three or more reviews in column 8.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 9  uses the coefficients from a specification that restricts the sample to Yelp reviewers with later-than-median dates of first review written.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 10 uses the coefficients from a specification that controls for origin-destination differences in vehicle ownership.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 11 uses the coefficients from a specification that controls for the number of Yelp reviews of each restaurant and its membership in a chain with more than eight NYC locations.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 12 uses the coefficients from a specification that introduces dummies for 39 more disaggregated cuisine categories.' >> ../output/tabsfigs/table_A15.tex
echo 'Column 13 uses the coefficients from columns four to six of Table \ref{tab:mainestimates} but constrains all trips to originate at home.' >> ../output/tabsfigs/table_A15.tex
echo 'See Tables \ref{tab:asian_robust1_sixom} through \ref{tab:whithisp_robust2_sixom} for these coefficients.\par}' >> ../output/tabsfigs/table_A15.tex
echo '\end{minipage}' >> ../output/tabsfigs/table_A15.tex
echo '\end{center}\end{table}'  >> ../output/tabsfigs/table_A15.tex

cp ../input/dissimilarity_venues_mintime.tex ../output/tabsfigs/table_A16.tex
...
echo '\begin{table} \caption{Isolation indices for various model specifications} \label{tab:Isolation-indices-appendix}' > ../output/tabsfigs/table_D1.tex
echo '\begin{center} \resizebox{\textwidth}{!}{' >> ../output/tabsfigs/table_D1.tex
cat  ../input/isoindices_sixom_robustness_ci.tex >> ../output/tabsfigs/table_D1.tex
echo '}' >> ../output/tabsfigs/table_D1.tex
echo '\begin{minipage}[t]{0.98\textwidth}' >> ../output/tabsfigs/table_D1.tex
echo '{\footnotesize \textsc{Notes}:' >> ../output/tabsfigs/table_D1.tex
echo 'The reported leave-out isolation indices $\hat{S}_{g}$, as defined in \citet{GentzkowShapiro:2011},' >> ../output/tabsfigs/table_D1.tex
echo 'are the values for the estimation sample and the 90\% confidence interval' >> ../output/tabsfigs/table_D1.tex
echo 'for model-predicted outcomes computed from 500 generated samples of the same size.' >> ../output/tabsfigs/table_D1.tex
echo 'The pooled model is Table \ref{tab:estimates:raceblind}.' >> ../output/tabsfigs/table_D1.tex
echo 'The race-specific model is columns four to six of Table \ref{tab:mainestimates}.' >> ../output/tabsfigs/table_D1.tex
echo "Nested 1 and Nested 2 are columns one to three and four to six of Table \ref{tab:nestedlogit}, respectively." >> ../output/tabsfigs/table_D1.tex
echo "Minimum time is the first column of Tables \ref{tab:asian_robust1_mintime}-\ref{tab:whithisp_robust1_mintime}. \par}" >> ../output/tabsfigs/table_D1.tex
echo '\end{minipage}\end{center}\end{table}' >> ../output/tabsfigs/table_D1.tex


echo '\begin{table} \caption{Restaurant fixed effects, estimated by \cite{Taddy:2015} procedure} \label{tab:Taddy-estimates} \vspace{-4mm} \begin{center}' > ../output/tabsfigs/table_D2.tex
cat  ../input/table_Taddy.tex >> ../output/tabsfigs/table_D2.tex
echo -e '\n \\begin{minipage}{.9\\textwidth}' >> ../output/tabsfigs/table_D2.tex
echo "{\footnotesize \textsc{Notes}: Each column reports an estimated conditional-logit model of individuals' decisions to visit a Yelp venue. Estimates computed per \cite{Taddy:2015} making the assumption that expected utility of a restaurant trip is equal across all individuals within a racial group.  Standard errors in parentheses. Statistical significance denoted by \textit{a} (1\%), \textit{b} (5\%), \textit{c} (10\%). The unreported covariates are restaurant fixed effects. \par}" >> ../output/tabsfigs/table_D2.tex
echo '\end{minipage}\end{center} \end{table}' >> ../output/tabsfigs/table_D2.tex

echo '\begin{table} \caption{Nested logit} \label{tab:nestedlogit}' > ../output/tabsfigs/table_D3.tex
echo '\begin{center} \vspace{-4mm} \resizebox{.84\textwidth}{!}{' >> ../output/tabsfigs/table_D3.tex
cat  ../input/table_nlogit_sixom.tex >> ../output/tabsfigs/table_D3.tex
echo '}' >> ../output/tabsfigs/table_D3.tex
echo '\begin{minipage}[t]{0.82\textwidth}' >> ../output/tabsfigs/table_D3.tex
echo '{\footnotesize \textsc{Notes}: Each column reports an estimated nested-logit model of the decision to visit a Yelp venue.' >> ../output/tabsfigs/table_D3.tex
echo 'Standard errors in parentheses. Statistical significance denoted by \textit{a} (1\%), \textit{b} (5\%), \textit{c} (10\%).' >> ../output/tabsfigs/table_D3.tex
echo 'Unreported controls are 28 area dummies.' >> ../output/tabsfigs/table_D3.tex
echo 'The $\chi^{2}$ test is a likelihood-ratio test for each specification relative to its corresponding entry in columns four to six of Table \ref{tab:mainestimates}.\par}' >> ../output/tabsfigs/table_D3.tex
echo '\end{minipage}' >> ../output/tabsfigs/table_D3.tex
echo '\end{center} \end{table}' >> ../output/tabsfigs/table_D3.tex

echo '\begin{figure} \caption{Parametric bootstrap: Social frictions and restaurant characteristics} \label{fig:bootstrap:socialcovariates} \vspace{-4mm}' > ../output/tabsfigs/figure_D2.tex
echo '\begin{center}' >> ../output/tabsfigs/figure_D2.tex
echo 'Asian reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mainspec_social_asian.pdf} \\' >> ../output/tabsfigs/figure_D2.tex
echo 'Black reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mainspec_social_black.pdf} \\' >> ../output/tabsfigs/figure_D2.tex
echo 'White/Hispanic reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mainspec_social_whithisp.pdf} \\' >> ../output/tabsfigs/figure_D2.tex
echo '\includegraphics[width=0.6\textwidth]{../input/bootstrap_param_legend.pdf}' >> ../output/tabsfigs/figure_D2.tex
echo '\end{center} \end{figure}' >> ../output/tabsfigs/figure_D2.tex

echo '\begin{figure} \caption{Parametric bootstrap: Spatial frictions} \label{fig:bootstrap:spatialfrictions} \vspace{-4mm}'  > ../output/tabsfigs/figure_D3.tex
echo '\begin{center}' >> ../output/tabsfigs/figure_D3.tex
echo 'Asian reviewers \\ \includegraphics[width=0.68\textwidth]{../input/bootstrap_param_mainspec_spatial_asian.pdf} \\' >> ../output/tabsfigs/figure_D3.tex
echo 'Black reviewers \\ \includegraphics[width=0.68\textwidth]{../input/bootstrap_param_mainspec_spatial_black.pdf} \\' >> ../output/tabsfigs/figure_D3.tex
echo 'White/Hispanic reviewers \\ \includegraphics[width=0.68\textwidth]{../input/bootstrap_param_mainspec_spatial_whithisp.pdf} \\' >> ../output/tabsfigs/figure_D3.tex
echo '\includegraphics[width=0.55\textwidth]{../input/bootstrap_param_legend.pdf}' >> ../output/tabsfigs/figure_D3.tex
echo '\end{center} \end{figure}' >> ../output/tabsfigs/figure_D3.tex

echo '\begin{figure} \caption{Parametric bootstrap: Social frictions and restaurant characteristics in minimum-time specification} \label{fig:bootstrap:mintime:socialcovariates}'  > ../output/tabsfigs/figure_D4.tex
echo '\begin{center}' >> ../output/tabsfigs/figure_D4.tex
echo 'Asian reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mintime_social_asian.pdf} \\' >> ../output/tabsfigs/figure_D4.tex
echo 'Black reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mintime_social_black.pdf} \\' >> ../output/tabsfigs/figure_D4.tex
echo 'White/Hispanic reviewers \\ \includegraphics[width=0.75\textwidth]{../input/bootstrap_param_mintime_social_whithisp.pdf} \\' >> ../output/tabsfigs/figure_D4.tex
echo '\includegraphics[width=0.6\textwidth]{../input/bootstrap_param_legend.pdf}' >> ../output/tabsfigs/figure_D4.tex
echo '\end{center} \end{figure}' >> ../output/tabsfigs/figure_D4.tex

echo '\begin{figure} \caption{Parametric bootstrap: Spatial frictions in minimum-time specification} \label{fig:bootstrap:mintime:spatialfrictions}'  > ../output/tabsfigs/figure_D5.tex
echo '\begin{center}' >> ../output/tabsfigs/figure_D5.tex
echo '\begin{minipage}{0.3\textwidth}\begin{center}Asian reviewers\end{center}\end{minipage}' >> ../output/tabsfigs/figure_D5.tex
echo '\begin{minipage}{0.3\textwidth}\begin{center}Black reviewers\end{center}\end{minipage}' >> ../output/tabsfigs/figure_D5.tex
echo '\begin{minipage}{0.3\textwidth}\begin{center}White/Hispanic reviewers\end{center}\end{minipage} \\' >> ../output/tabsfigs/figure_D5.tex
echo '\includegraphics[width=0.9\textwidth]{../input/bootstrap_param_mintime_spatial.pdf}' >> ../output/tabsfigs/figure_D5.tex
echo '\includegraphics[width=0.6\textwidth]{../input/bootstrap_param_legend.pdf}' >> ../output/tabsfigs/figure_D5.tex
echo '\end{center} \end{figure}' >> ../output/tabsfigs/figure_D5.tex

echo '\begin{table} \caption{Parametric bootstrap: Isolation index confidence intervals} \label{tab:isoindex:bootstrapavg} \begin{center}'  > ../output/tabsfigs/table_D4.tex
cat  ../input/isoindices_bsavg_ci.tex >> ../output/tabsfigs/table_D4.tex
echo ' \end{center}\end{table}'  >> ../output/tabsfigs/table_D4.tex

echo '\begin{figure} \caption{Parametric bootstrap: Isolation index confidence intervals} \label{fig:bootstrap:isoindices_p5p95}'  > ../output/tabsfigs/figure_D6.tex
echo '\begin{center} \includegraphics{../input/isoindices_p5p95distros.pdf} ' >> ../output/tabsfigs/figure_D6.tex
echo '\begin{minipage}{.7\textwidth}{\footnotesize \textsc{Notes}: Each panel depicts the distribution of the endpoints of the 90\% confidence interval for the isolation index obtained from the 496 bootstrap draws. The vertical lines depict the endpoints of the 90\% confidence interval reported in Table \ref{tab:Model-fit:Isolation-indices}. \par}\end{minipage}' >> ../output/tabsfigs/figure_D6.tex
echo '\end{center}\end{figure}' >> ../output/tabsfigs/figure_D6.tex

echo '\begin{figure}[!h] \caption{Dissimilarity indices for bootstrapped distribution of estimates} \label{fig:dissimilaritybootstrapplots:alltogether}' > ../output/tabsfigs/figure_D7.tex
echo '\begin{center}' >> ../output/tabsfigs/figure_D7.tex
echo '\includegraphics{../input/dissimilarity_bootstrap_plot.pdf}' >> ../output/tabsfigs/figure_D7.tex
echo '\begin{minipage}{.8\textwidth}' >> ../output/tabsfigs/figure_D7.tex
echo '{\footnotesize \textsc{Notes}: Epanechnikov kernel with bandwidth of .001. Vertical lines denote estimated values reported in Table \ref{tab:dissimilarity:bootstrap}, upper panel, column 2.\par}' >> ../output/tabsfigs/figure_D7.tex
echo '\end{minipage} \end{center} \end{figure}' >> ../output/tabsfigs/figure_D7.tex

cp ../input/dissimilarity_bootstrap_table.tex ../output/tabsfigs/table_D5.tex 
